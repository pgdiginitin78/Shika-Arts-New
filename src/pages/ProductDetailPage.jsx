...
function ProductDetailPage() {
  ...
  const handleAdd = async () => {
    if (!variant) return;
    await addItem({ productId: node.id, handle: node.handle, product: node, quantity: qty });
    setOpen(true);
  };
  ...
  const PERSONALIZATIONS = ["Name Engraving", "Date Engraving"];
        ...
        <div className="overflow-hidden bg-white rounded-[10px] shadow-sm border border-gray-100">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
            <div className="flex flex-col sm:flex-row gap-2 p-3 sm:p-4">
              <div className="flex sm:flex-col gap-2 order-2 sm:order-1 sm:w-[50px] shrink-0 overflow-x-auto sm:overflow-y-auto sm:max-h-[350px] pb-1 sm:pb-0 pr-0 sm:pr-1">
                {allImages.length > 0
                  ? allImages.map((img, i) => (
                      <button
                        key={i}
              ...
              <div className="relative flex-1 order-1 sm:order-2 aspect-square sm:aspect-auto sm:min-h-[300px] overflow-hidden rounded-lg bg-gray-50">
                {currentImage ? (
                  <img
            ...
            <div className="flex flex-col px-3 sm:px-4 lg:px-6 py-4 border-t lg:border-t-0 lg:border-l border-gray-100">
                  ...
                  {variant.regularPrice > 0 &&
                    Number(variant.regularPrice) !== Number(variant.price.amount) && (
                      <span className="text-xs text-gray-400 line-through">
                        {formatPrice(Number(variant.regularPrice), variant.price.currencyCode)}
                      </span>
                    )}
                </div>
              )}
              ...
              <div className="mb-3">
                ...
                <div className="flex flex-row gap-2">
                  ...
                  <button
                    ...
                    className="flex-1 h-9 cursor-pointer flex items-center justify-center gap-1.5 bg-[#1e2321] text-white rounded text-[10px] font-bold uppercase tracking-widest hover:bg-[#2d3532] transition-colors disabled:opacity-50"
                  >
                    {isLoading ? (
              ...
              <div className="border-b border-gray-100">
                ...
                <AccordionItem icon={<Truck size={14} />} label="Shipping & Delivery">
                  Same-day delivery available for orders placed before 2 PM. Standard delivery in
                  3–5 business days. Express delivery available at checkout.
                </AccordionItem>
