...
export function StatCard({
  ...
  const display = format ? format(counted) : new Intl.NumberFormat("en-IN").format(counted);
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-2xl border border-stone-200 bg-white p-3.5 shadow-sm transition-shadow duration-300 hover:shadow-lg sm:p-5"
    >
      <div
        className="pointer-events-none absolute -right-6 -top-8 h-28 w-28 rounded-full opacity-[0.16]"
      ...
      <div className="relative flex items-start justify-between gap-2">
        <span className="text-[10px] font-semibold uppercase tracking-wider text-stone-400 sm:text-[11px]">
          {label}
        </span>
        <div className="relative h-9 w-9 shrink-0 sm:h-11 sm:w-11">
          <div
            className="absolute inset-0 rounded-full border border-dashed opacity-45"
            style={{ borderColor: palette.to }}
          />
          <div
            className="absolute inset-[3px] rounded-full"
            style={{
              background: `linear-gradient(145deg, ${palette.from}, ${palette.to})`,
              boxShadow: `0 6px 14px -4px ${palette.glow}`,
            }}
          />
      ...
      <p
        className="relative mt-3 text-[20px] font-bold leading-none text-stone-900 sm:text-[27px]"
        style={{ fontVariantNumeric: "tabular-nums" }}
      >
        {display}
      </p>

      <p className="relative mt-2 text-[11px] text-stone-400">{caption}</p>
    </motion.div>
  );
}
