...
function getResponsiveValues() {
  ...
  if (w < 480)
    return { size: 44, sparkDist: [14, 10], sparkSize: [2, 2], sparkCount: 7, duration: "0.75s" };
  if (w < 768)
    return { size: 52, sparkDist: [16, 12], sparkSize: [2, 3], sparkCount: 8, duration: "0.80s" };
  if (w < 1024)
    return { size: 58, sparkDist: [18, 14], sparkSize: [3, 3], sparkCount: 9, duration: "0.85s" };
  if (w < 1440)
    return { size: 64, sparkDist: [20, 16], sparkSize: [3, 4], sparkCount: 10, duration: "0.90s" };
  if (w < 1920)
    return { size: 70, sparkDist: [22, 18], sparkSize: [3, 4], sparkCount: 10, duration: "0.90s" };
  if (w < 2560)
    return { size: 80, sparkDist: [26, 20], sparkSize: [4, 5], sparkCount: 12, duration: "0.95s" };
  if (w < 3840)
    return { size: 92, sparkDist: [30, 24], sparkSize: [5, 6], sparkCount: 12, duration: "1.00s" };
  if (w < 7680)
    return { size: 110, sparkDist: [36, 28], sparkSize: [6, 7], sparkCount: 14, duration: "1.05s" };
  ...
  return (
    <CartAnimationContext.Provider value={{ launchAnimation }}>
      ...
      {createPortal(
        <>
          {flyItems.map((item) => (
            <FlyingImage key={item.id} item={item} onLanded={handleLanded} rv={item.rv} />
          ))}
        </>,
        document.body,
      )}
