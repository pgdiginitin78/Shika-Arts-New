...
export function Header() {
  ...
  const handleLogin = async () => {
    if (SHOPIFY_CLIENT_ID) {
      // Full OAuth PKCE flow — supports OTP, passwordless, Shop login
      try {
        await initiateLogin(SHOPIFY_CLIENT_ID);
      } catch (err) {
        console.error("Login failed:", err);
      }
    } else {
      // Show login modal (email/password via Storefront API)
      setIsLoginOpen(true);
    }
  };
  ...
  const handleLogout = async () => {
    ...
    if (SHOPIFY_CLIENT_ID && idToken) {
      try {
        await initiateLogout(SHOPIFY_CLIENT_ID, idToken);
        // Shopify redirects to https://shika-arts.vercel.app after logout
      } catch {
        navigate("/");
      }
    } else {
